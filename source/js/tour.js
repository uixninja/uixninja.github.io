jQuery(document).ready(function($){
	...
	function initTour() {
		...
		tourStepInfo.on('click', '.cd-close-2', function(event){
			closeTour(tourSteps, tourWrapper, coverLayer);
		});
	...
	function showStep(step, layer) {
		...
		$($(step).attr('data-active-selector')).addClass('active-overlay');
		
		$('body').attr('data-step', $(step).find('.cd-actual-step').html());
